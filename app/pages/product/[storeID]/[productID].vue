            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-12">
                ...
                <div class="col-span-1 lg:col-span-2">
                    <div class="gap-4">
                        ...
                        <h1 class="text-primary text-2xl sm:text-3xl font-bebas-neue mt-2">{{ product?.name }}</h1>
                        <p class="mt-1 text-gray-700">Rp. {{ product?.price }}</p>

                        <div>
                            <p class="mb-2 mt-5 pt-3">Pilih Varian</p>
                            <div class="flex flex-wrap gap-6">
                                <button v-for="varian in produkVarian" @click="varianTerpilih = varian" :class="[
                                    'rounded-full border px-5 py-3 text-sm font-medium hover:text-white hover:bg-orange-400',
                                    varianTerpilih === varian
                                        ? 'border-orange-400 bg-orange-400 text-white'
                                        : 'border-orange-400 border-2 text-gray-700',
                                ]">
                                    {{ varian }}
                                </button>
                            </div>
                        ...
                        <div class="flex items-center gap-3 pt-4">
                            <button
                                class="flex-grow rounded-full bg-green-600 px-6 py-3 font-semibold text-white shadow-sm transition hover:bg-green-700">
                                Masukkan ke Keranjang
                            </button>
                            <button
                                class="rounded-full border border-gray-300 p-3 text-gray-500 transition hover:border-red-500 hover:text-red-500">
                ...
                <div class="col-span-1">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 justify-center items-center">
                        <div class="flex items-end justify-center sm:justify-start">
                            <span class="text-5xl sm:text-7xl font-bebas-neue text-primary">{{ product?.rating.average }}</span>
            ...
            <div class="bg-stone-50 py-5">
                <div class="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-3 gap-8">
                        ...
                        <div v-for="(review,index) in product?.reviews" :key="index" class="block p-6 border border-gray-300 rounded-lg">
                            <div class="">
                                <span class="font-semibold">{{ review.user }}</span>
                                <div class="flex items-center justify-between">
                                    <div class="flex text-yellow-500">
                                        <svg v-for="i in review.stars" :key="i" class="h-4 w-4" viewBox="0 0 20 20"
                                            fill="currentColor">
                                            <path
                                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.974a1 1 0 00.95.69h4.182c.969 0 1.371 1.24.588 1.81l-3.38 2.454a1 1 0 00-.364 1.118l1.287 3.974c.3.921-.755 1.688-1.539 1.118l-3.38-2.454a1 1 0 00-1.175 0l-3.38 2.454c-.784.57-1.838-.197-1.539-1.118l1.287-3.974a1 1 0 00-.364-1.118L2.34 9.401c-.783-.57-.381-1.81.588-1.81h4.182a1 1 0 00.95-.69L9.049 2.927z" />
                                        </svg>
...
// Pastikan impor komponen Anda benar
import SecondHeader from '~/components/SecondHeader.vue'; 
import Carousel from '~/components/Carousel.vue';       
import TheFooter from '~/components/TheFooter.vue';     
// Pastikan path ini benar (seperti yang kita diskusikan, 'assets' lebih baik)
import allStoresData from '~/data/products.json'; 

// Hapus 'defineProps()' dari sini, ini adalah halaman, bukan komponen

const route = useRoute();
const { storeID, productID } = route.params;

// --- 1. Cek parameter URL ---
console.log('Parameter URL storeID:', storeID, '(Tipe:', typeof storeID, ')');
console.log('Parameter URL productID:', productID, '(Tipe:', typeof productID, ')');

const store = allStoresData.find(s => String(s.id) === storeID);

// --- 2. Cek apakah store ditemukan ---
console.log('Store yang ditemukan:', storeID);

const product = store ? store.product.find(p => String(p.id) === productID) : null;

// --- 3. Cek apakah produk ditemukan ---
console.log('Produk yang ditemukan:', product);


// Cek sisa variabel Anda
const produkVarian = product ? product.variants : []; 
const varianTerpilih = ref(produkVarian.length > 0 ? produkVarian[0] : '');

console.log('Varian yang tersedia:', produkVarian);
console.log('Varian terpilih:', varianTerpilih.value); // Gunakan .value untuk melihat isi ref
