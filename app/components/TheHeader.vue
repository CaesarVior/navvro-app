<template>
  <header :class="['bg-transparent sticky top-0 z-50 relative', scrolled ? 'shadow bg-white' : '']">
    <div class="mx-auto container flex items-center h-16"> <div class="flex-1">
      ...
      <div class="flex-1 flex justify-end items-center space-x-3 py-8">
        
        <a v-if="!isLoggedIn" href="/login"
          class="text-white bg-[#FF8A00] rounded-lg text-center block px-8 py-2 font-poppins text-sm hover:bg-orange-400">
          Login
        </a>
        <div v-else class="flex items-center ">
          <a href="/profile" aria-label="Lihat Profil"
            class="bg-[#FF8A00] hover:bg-orange-500 rounded-full w-8 h-8 flex items-center justify-center transition-colors">
